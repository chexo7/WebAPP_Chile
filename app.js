document.addEventListener('DOMContentLoaded', () => {
    ...
    function mapJsonTransactionsToExpenses(obj) {
        ...
        (obj.PostedTransactions || []).forEach(tx => {
            ...
            if (!description || !dateObj || withdrawal === null || isNaN(withdrawal)) return;
            ...
            results.push({
                ...
                amount,
                currency: 'USD',
                sourceType: tx.Type || tx.type || 'OTRO',
                isPending: false,
    ...
    function handleJsonExpensesText(text) {
        ...
        if (!parsedJsonExpenses.length) {
            jsonExpensesPreview.textContent = 'No se encontraron movimientos con descripción, fecha y monto (solo PostedTransactions con Withdrawal).';
            showJsonExpensesModal();
            return;
        }
    ...
    function addJsonExpenses() {
        ...
        parsedJsonExpenses.forEach((item, idx) => {
            if (item.amount === null || isNaN(item.amount)) { return; }
