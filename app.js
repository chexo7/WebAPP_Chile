document.addEventListener('DOMContentLoaded', () => {
    ...
    const budgetDeletionList = document.getElementById('budget-deletion-list');
    ...
    function renderBudgetDeletionList() {
        if (!budgetDeletionList) return;
        budgetDeletionList.innerHTML = '';
        if (!currentBackupData || !currentBackupData.budgets || Object.keys(currentBackupData.budgets).length === 0) {
            const emptyItem = document.createElement('li');
            emptyItem.className = 'budget-deletion-empty';
            emptyItem.textContent = 'No hay presupuestos definidos.';
            budgetDeletionList.appendChild(emptyItem);
            return;
        }
        const targetDate = (currentBudgetViewDate instanceof Date) ? currentBudgetViewDate : new Date();
        const sortedCategories = Object.keys(currentBackupData.budgets).sort();
        sortedCategories.forEach(catName => {
            const budgetEntry = normalizeBudgetEntry(currentBackupData.budgets[catName]);
            const monthlyBudget = calculateMonthlyBudgetValue(budgetEntry, targetDate);
            const item = document.createElement('li');
            item.className = 'budget-deletion-item';

            const info = document.createElement('div');
            info.className = 'budget-deletion-info';
            const title = document.createElement('strong');
            title.textContent = catName;
            const meta = document.createElement('div');
            meta.className = 'budget-deletion-meta';
            const amountText = `${formatCurrencyJS(budgetEntry.amount, currentBackupData.display_currency_symbol || '$')} · ${budgetEntry.frequency}`;
            const monthlyText = `Mes seleccionado: ${formatCurrencyJS(monthlyBudget, currentBackupData.display_currency_symbol || '$')}`;
            meta.textContent = `${amountText} · ${monthlyText}`;
            ...
            budgetDeletionList.appendChild(item);
        });
    }
