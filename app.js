document.addEventListener('DOMContentLoaded', () => {
    ...
    const GEMINI_CONTEXT = "Eres un asistente para la aplicaci\u00f3n financiera que administra gastos e ingresos. " +
        "Cada gasto se guarda con los campos name, amount, category, frequency, movement_date, start_date, end_date, payment_method, credit_card, installments e is_real. " +
        "Cuando recibas un archivo Excel para importar, debes detectar duplicados y tambi\u00e9n identificar las columnas de fecha, descripci\u00f3n y monto. " +
        "Si se te solicita el mapeo de columnas, responde \u00fanicamente con un JSON del tipo {\\\"date\\\":\\\"...\\\", \\\"desc\\\":\\\"...\\\", \\\"amount\\\":\\\"...\\\"}.";
    ...
    async function analyzeMappingWithAI() {
        if (!aiAvailable) return null;
        const sample = parsedImportData.slice(0, 3);
        const prompt = `Columnas: ${JSON.stringify(importHeaders)}. Ejemplos: ${JSON.stringify(sample)}. Devuelve solo un JSON del tipo {date:'col', desc:'col', amount:'col'} usando exactamente los nombres de las columnas.`;
        const reply = await geminiRequest(prompt);
    ...
    async function applyBankProfile(profileKey) {
        ...
        if (profile && profile.columns) {
            ...
            if (aiAvailable) {
                try {
                    mapping = await analyzeMappingWithAI();
                } catch (e) { console.error(e); }
            }
            if (mapping) {
                mapDateSelect.value = importHeaders.find(h => h === mapping.date) || '';
                mapDescSelect.value = importHeaders.find(h => h === mapping.desc) || '';
                mapAmountSelect.value = importHeaders.find(h => h === mapping.amount) || '';
            } else {
    ...
    if (bankProfileSelect) bankProfileSelect.addEventListener('change', () => { applyBankProfile(bankProfileSelect.value).then(requestAIDuplicates); });
