document.addEventListener('DOMContentLoaded', () => {
    ...
    async function exportMonthlyReportToExcel() {
        ...
        const setSheetColumnWidths = (sheet, columnWidths) => {
            if (!sheet) return;
        ...
        const buildCategorySheet = (categories, sheetName) => {
            ...
            setSheetColumnWidths(sheet, [32, ...monthLabels.map(() => 18)]);
        ...
        const buildCategoryBreakdownSheet = (categories, sheetName) => {
            ...
            categories.forEach(cat => {
                ...
                rows.push(Array(monthLabels.length + 1).fill(''));
            });
            const sheet = buildSheet(rows);
            applyNumberFormatToSheet(sheet);
            setSheetColumnWidths(sheet, [42, ...monthLabels.map(() => 18)]);
            XLSX.utils.book_append_sheet(wb, sheet, sheetName);
        };
