document.addEventListener('DOMContentLoaded', () => {
    ...
    const jsonExpenseDropZone = document.getElementById('json-expense-drop-zone');
    ...
    const gastosTabContent = document.getElementById('gastos');
    ...
    function createCategorySelect(includeEmpty = false) {
    ...
    function showJsonExpensesModal(preserveInput = false) {
        parsedJsonExpenses = [];
        if (jsonExpensesInput && !preserveInput) jsonExpensesInput.value = '';
    ...
    function mapJsonTransactionsToExpenses(obj) {
        ...
        const pushTx = (tx) => {
            ...
            seen.add(key);
            results.push({ name: description, amount, date: dateObj, category: '' });
        };
    ...
    function renderJsonExpensesPreview() {
        ...
        parsedJsonExpenses.forEach((item, idx) => {
            ...
            const sel = createCategorySelect(true);
    ...
    function syncJsonExpenseCategories() {
        if (!jsonExpensesPreview) return;
        const selects = jsonExpensesPreview.querySelectorAll('select[data-index]');
        selects.forEach(sel => {
            const idx = parseInt(sel.dataset.index, 10);
            if (!isNaN(idx) && parsedJsonExpenses[idx]) {
                parsedJsonExpenses[idx].category = sel.value;
            }
        });
    }
    ...
    function addJsonExpenses() {
        ...
        syncJsonExpenseCategories();
        if (parsedJsonExpenses.some(item => !item.category)) { alert('Asigna una categoría para cada gasto.'); return; }
        if (!currentBackupData.expenses) currentBackupData.expenses = [];
        ...
        parsedJsonExpenses.forEach(item => {
            ...
            const expense = {
                ...
                type: currentBackupData.expense_categories ? currentBackupData.expense_categories[item.category] || 'Variable' : 'Variable',
                frequency: 'Único',
    ...
    function handleJsonExpenseFile(file) {
        if (!file) return;
        if (!/\.json$/i.test(file.name)) { alert('Solo se aceptan archivos .json'); return; }
        const reader = new FileReader();
        reader.onload = (e) => {
            if (!jsonExpensesInput) return;
            showJsonExpensesModal(true);
            jsonExpensesInput.value = e.target.result;
            previewJsonExpenses();
        };
        reader.readAsText(file);
    }
    ...
    if (jsonExpenseDropZone) {
        jsonExpenseDropZone.addEventListener('dragover', e => { e.preventDefault(); jsonExpenseDropZone.classList.add('dragover'); });
        jsonExpenseDropZone.addEventListener('dragleave', () => jsonExpenseDropZone.classList.remove('dragover'));
        jsonExpenseDropZone.addEventListener('drop', e => {
            e.preventDefault();
            jsonExpenseDropZone.classList.remove('dragover');
            const file = e.dataTransfer.files && e.dataTransfer.files[0];
            handleJsonExpenseFile(file);
        });
    }
    if (gastosTabContent) {
        gastosTabContent.addEventListener('dragover', e => {
            if (e.dataTransfer && e.dataTransfer.types.includes('Files')) {
                e.preventDefault();
                if (jsonExpenseDropZone) jsonExpenseDropZone.classList.add('dragover');
            }
        });
        gastosTabContent.addEventListener('dragleave', () => {
            if (jsonExpenseDropZone) jsonExpenseDropZone.classList.remove('dragover');
        });
        gastosTabContent.addEventListener('drop', e => {
