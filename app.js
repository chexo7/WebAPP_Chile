document.addEventListener('DOMContentLoaded', () => {
    ...
    let appLoadingProgressValue = 0;
    let appLoadingTimer = null;
    function waitMs(ms = 0) {
        return new Promise(resolve => setTimeout(resolve, ms));
    }

    function startAppLoadingOverlay(title = 'Cargando información') {
        if (!appLoadingOverlay) return;
        if (appLoadingTimer) clearInterval(appLoadingTimer);
        appLoadingProgressValue = 6;
        if (appLoadingTitle) appLoadingTitle.textContent = title;
        if (appLoadingDescription) appLoadingDescription.textContent = 'Preparando datos...';
        if (appLoadingProgressBar) appLoadingProgressBar.style.width = `${appLoadingProgressValue}%`;
        showElement(appLoadingOverlay, 'flex');
        appLoadingTimer = setInterval(() => {
            if (appLoadingProgressValue < 92) {
                appLoadingProgressValue += Math.random() * 2.8;
                if (appLoadingProgressBar) appLoadingProgressBar.style.width = `${Math.min(92, appLoadingProgressValue)}%`;
            }
        }, 180);
    }

    function updateAppLoadingOverlay(progress, description) {
        if (!appLoadingOverlay || appLoadingOverlay.style.display === 'none') return;
        appLoadingProgressValue = Math.max(appLoadingProgressValue, Math.min(99, progress));
        if (appLoadingProgressBar) appLoadingProgressBar.style.width = `${appLoadingProgressValue}%`;
        if (appLoadingDescription && description) appLoadingDescription.textContent = description;
    }

    async function finishAppLoadingOverlay(description = 'Todo listo, cargando vista principal...') {
        if (!appLoadingOverlay) return;
        if (appLoadingTimer) {
            clearInterval(appLoadingTimer);
            appLoadingTimer = null;
        }
        updateAppLoadingOverlay(100, description);
        await waitMs(260);
        hideElement(appLoadingOverlay);
    }

    function cancelAppLoadingOverlay() {
        if (appLoadingTimer) {
            clearInterval(appLoadingTimer);
            appLoadingTimer = null;
        }
    ...
    const appLoadingProgressBar = document.getElementById('app-loading-progress-bar');
    ...
    async function applyBackupDataToApp(backupData, key = null, options = {}) {
        if (!backupData) {
            return;
        }

        const { skipOverlayStart = false } = options;
        if (!skipOverlayStart) {
            startAppLoadingOverlay('Preparando tu información financiera');
        }

        updateAppLoadingOverlay(14, 'Validando y organizando datos...');
        ...
        await waitMs(70);
        updateAppLoadingOverlay(34, 'Renderizando tablas principales...');
        ...
        await waitMs(70);
        updateAppLoadingOverlay(52, 'Configurando períodos y selectores...');
        ...
        updateAppLoadingOverlay(74, 'Calculando flujo de caja...');
        await renderCashflowTable();

        updateAppLoadingOverlay(90, 'Creando gráficos y últimos ajustes...');
        updateAnalysisModeLabels();

        hideElement(dataSelectionContainer);
        hideElement(loadingMessage);
        showMainContentScreen();
        await finishAppLoadingOverlay('Listo, ya puedes usar la aplicación.');
    }
    ...
    async function loadSpecificBackup(key) {
        ...
        clearAllDataViews();
        originalLoadedData = null;
        startAppLoadingOverlay('Cargando tu versión de datos');
        updateAppLoadingOverlay(10, 'Descargando versión desde la base de datos...');

        try {
            const snapshot = await userRef.child(`backups/${key}`).once('value');
            if (snapshot.exists()) {
                updateAppLoadingOverlay(24, 'Procesando versión seleccionada...');
                const hydratedData = hydrateBackupData(snapshot.val());
                await applyBackupDataToApp(hydratedData, key, { skipOverlayStart: true });
                if (backupSelector) {
                    backupSelector.value = key;
                }
            } else {
                updateAppLoadingOverlay(24, 'No hubo datos en la versión. Generando base inicial...');
                ...
                await applyBackupDataToApp(defaultData, null, { skipOverlayStart: true });
            }
        } catch (error) {
