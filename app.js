document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyDatasetVisibilityFromToggles(shouldUpdate = true) {
        ...
        chartDatasetToggles.forEach(toggle => {
            ...
            cashflowChartInstance.data.datasets.forEach(ds => {
                if (ds.label === targetLabel) ds.hidden = !toggle.checked;
            });
        });
    ...
    function renderCashflowChart(periodDates, incomes, totalExpenses, netFlows, endBalances, dailyLine = null, storeData = true, rangeKey = null) {
        ...
        const showPeriodMarkers = activeCashflowPeriodicity !== 'Diario';
        ...
        const periodMarkerPoints = showPeriodMarkers ? periodDates.map((date, idx) => ({ x: date.getTime(), y: endBalances[idx] })) : [];
