document.addEventListener('DOMContentLoaded', () => {
    ...
    const GEMINI_CONTEXT = "Eres un asistente para la aplicaci\u00f3n financiera que administra gastos e ingresos. Cada gasto se guarda con los campos name, amount, category, frequency, movement_date, start_date, end_date, payment_method, credit_card, installments e is_real.";
    const GEMINI_MAP_INSTRUCTION = "Analiza la siguiente tabla proveniente de un archivo de gastos. Tu tarea es identificar qué columna corresponde a la fecha, cuál a la descripción y cuál al monto. Responde únicamente con un objeto JSON con las claves 'date', 'description' y 'amount' usando exactamente los encabezados proporcionados.";
    ...
    async function analyzeMappingWithAI(headers, sampleRows) {
        ...
        const lines = [headers, ...sampleRows].map(r => r.join(' | ')).join('\n');
        const prompt = `${GEMINI_MAP_INSTRUCTION}\n${lines}`;
        const reply = await geminiRequest(prompt);
    ...
    async function requestAIMapping() {
        ...
        const sampleRows = parsedImportData.slice(0, 3).map(row => importHeaders.map(h => row[h]));
        const map = await analyzeMappingWithAI(importHeaders, sampleRows);
        if (map) {
            if (map.date) {
                const match = importHeaders.find(h => h.toLowerCase() === map.date.toLowerCase());
                if (match) mapDateSelect.value = match;
            }
            if (map.description) {
                const match = importHeaders.find(h => h.toLowerCase() === map.description.toLowerCase());
                if (match) mapDescSelect.value = match;
            }
            if (map.amount) {
                const match = importHeaders.find(h => h.toLowerCase() === map.amount.toLowerCase());
                if (match) mapAmountSelect.value = match;
            }
        }
