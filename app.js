document.addEventListener('DOMContentLoaded', () => {
    ...
    async function renderCashflowTableFor(periodicity, tableHeadEl, tableBodyEl, titleEl) {
        ...
        let analysisStart = new Date(monthStart.getTime());
        ...
        periodDates.forEach(d => {
            ...
            if (periodicity === 'Semanal') {
                ...
                th.innerHTML = `${day} ${MONTH_NAMES_ES[d.getUTCMonth()]}<br>${d.getUTCFullYear()}`;
            } else {
...
function getMondayOfWeek(year, week) {
    ...
    function highlightCurrentPeriodColumn(periodicity, headEl, bodyEl, periodDates) {
        ...
        for (let i = 0; i < periodDates.length; i++) {
            ...
            if (today >= start && today < exclusiveEnd) { idx = i; break; }
        }
        if (idx === -1) return;
