document.addEventListener('DOMContentLoaded', () => {
    ...
    function syncRangeSliderFromWindow() {
        if (!chartRangeSlider || !chartDataDomain || !chartViewWindow) return;
        const domainSpan = chartDataDomain.max - chartDataDomain.min;
        const viewSpan = chartViewSpanMs ?? (chartViewWindow.end.getTime() - chartViewWindow.start.getTime());
        const effectiveSpan = Math.max(0, domainSpan - viewSpan);
        if (viewSpan <= 0 || domainSpan <= 0 || effectiveSpan <= 0) {
            chartRangeSlider.disabled = true;
            chartRangeSlider.value = 0;
            return;
        }
        const startPos = chartViewWindow.start.getTime() - chartDataDomain.min;
        const ratio = Math.min(1, Math.max(0, startPos / effectiveSpan));
        chartRangeSlider.disabled = false;
        chartRangeSlider.value = (ratio * 100).toFixed(2);
    }

    function applyRangeSliderValue(value) {
        if (!chartDataDomain || chartRangeSlider.disabled) return;
        const domainSpan = chartDataDomain.max - chartDataDomain.min;
        const viewSpan = chartViewSpanMs ?? (chartViewWindow ? (chartViewWindow.end.getTime() - chartViewWindow.start.getTime()) : domainSpan);
        const effectiveSpan = Math.max(0, domainSpan - viewSpan);
        if (effectiveSpan <= 0) return;
        const ratio = Math.min(1, Math.max(0, value / 100));
        const startMs = chartDataDomain.min + ratio * effectiveSpan;
        const endMs = Math.min(chartDataDomain.max, startMs + viewSpan);
        const targetWindow = { start: new Date(startMs), end: new Date(endMs) };
        chartViewWindow = targetWindow;
        chartViewSpanMs = viewSpan;
        lastChartRangeKey = 'slider';
...
function getMondayOfWeek(year, week) {
    ...
    if (chartRangeSlider) {
        chartRangeSlider.addEventListener('input', (e) => {
            const value = parseFloat(e.target.value);
            if (!isNaN(value)) applyRangeSliderValue(value);
        });
    }
