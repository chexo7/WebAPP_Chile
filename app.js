document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetPlannedExpenseForm() {
        ...
        if (plannedExpenseStartDateInput) {
            const baseDate = currentBudgetViewDate instanceof Date && !isNaN(currentBudgetViewDate)
                ...
                : (currentBackupData && currentBackupData.analysis_start_date
                    ? new Date(currentBackupData.analysis_start_date)
                    : new Date());
    ...
    function renderPlannedExpensesTable() {
        ...
        sortedPlans.forEach(plan => {
            ...
            const plannedDateWrapper = document.createElement('div');
            plannedDateWrapper.classList.add('planned-expense-date');

            const plannedBaseDate = plan.start_date ? getISODateString(toUTCDate(plan.start_date)) : null;
            const baseLabel = document.createElement('div');
            baseLabel.textContent = plannedBaseDate
                ? `Fecha presupuestada: ${plannedBaseDate}`
                : 'Define una fecha presupuestada de pago';
            plannedDateWrapper.appendChild(baseLabel);

            const helper = document.createElement('div');
            helper.classList.add('small-text');
            if (occurrenceDate) {
                helper.textContent = `Próximo registro: ${getISODateString(occurrenceDate)} (${occurrenceKey || selectedPeriodKey})`;
                if (occurrenceKey && occurrenceKey !== selectedPeriodKey) {
                    helper.textContent += ` · Programado para ${occurrenceKey}`;
                }
            } else {
                helper.textContent = 'No se pudo calcular la siguiente fecha. Revisa la fecha presupuestada.';
            }
            plannedDateWrapper.appendChild(helper);
            nextDateCell.appendChild(plannedDateWrapper);
