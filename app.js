document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateChartSliderFromView() {
        if (!chartRangeSlider || !chartDataDomain || chartViewSpanMs === null || !chartViewWindow) return;
        const domainSpan = chartDataDomain.max - chartDataDomain.min;
        const movableSpan = Math.max(0, domainSpan - chartViewSpanMs);
        if (movableSpan === 0) {
        ...
        const startOffset = chartViewWindow.start.getTime() - chartDataDomain.min;
        const pct = Math.min(1, Math.max(0, startOffset / movableSpan));
        chartRangeSlider.value = Math.round(pct * 100).toString();
    }
...
function getMondayOfWeek(year, week) {
    ...
    if (chartRangeSlider) {
        chartRangeSlider.addEventListener('input', () => {
            ...
            const pct = Math.min(1, Math.max(0, parseInt(chartRangeSlider.value, 10) / 100));
            const startMs = chartDataDomain.min + pct * movableSpan;
            const endMs = startMs + chartViewSpanMs;
            applyViewportToChart({ start: new Date(startMs), end: new Date(endMs) });
        });
    }
