document.addEventListener('DOMContentLoaded', () => {
    ...
    async function exportMonthlyReportToExcel() {
        ...
        const appendLineChartIfSupported = (workbook, sheetName, chartConfig) => {
            if (!XLSX || !XLSX.utils) return;
            if (typeof XLSX.utils.book_append_chart === 'function') {
                XLSX.utils.book_append_chart(workbook, sheetName, chartConfig);
                return;
            }
            if (typeof XLSX.utils.sheet_add_chart === 'function') {
                const sheet = workbook.Sheets[sheetName];
                if (sheet) {
                    XLSX.utils.sheet_add_chart(sheet, chartConfig);
                }
            }
        };
        ...
        const diarioRowCount = dailyRows.length;
        const diarioRangeX = `'Diario'!$A$2:$A$${diarioRowCount}`;
        const diarioRangeY = `'Diario'!$F$2:$F$${diarioRowCount}`;
        appendLineChartIfSupported(wb, 'Resumen', {
            title: 'Saldo final diario',
            anchor: { from: 'A9', to: 'R33' },
            type: 'line',
            subtype: 'lineMarkers',
            series: [
                {
                    name: 'Saldo final',
                    categories: diarioRangeX,
                    values: diarioRangeY
                }
            ]
        });
