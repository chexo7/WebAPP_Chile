    ...
    function showBreakdownPopup(cell) {
        ...
        if (!rows || rows.length === 0) {
            ...
            const items = rows.map(r => {
                if (r.type === 'Reembolso') {
                    return `<li><span class="desc">Reembolso</span><span class="amt">${r.date}</span></li>`;
                }
                const amt = formatCurrencyJS(r.type === 'Gasto' && r.originalAmount ? r.originalAmount : r.amount, symbol);
                const sign = r.type === 'Gasto' ? '-' : '+';
                total += r.type === 'Gasto' ? -r.amount : r.amount;
                return `<li><span class="desc">${r.name}</span><span class="amt">${sign} ${amt}</span></li>`;
            }).join('');
            breakdownPopup.innerHTML = `<ul>${items}</ul><strong>Total: ${formatCurrencyJS(total, symbol)}</strong>`;
        }
        breakdownPopup.style.display = 'block';
        breakdownPopup.style.width = 'max-content';
        const rect = cell.getBoundingClientRect();
        const popupWidth = breakdownPopup.offsetWidth;
        breakdownPopup.style.left = (rect.left + window.scrollX + (rect.width - popupWidth) / 2) + 'px';
        ...
        (currentBackupData.incomes || []).forEach(inc => {
            ...
            if (occ > 0) {
                ...
                rows.push({
                    ...
                    isReimbursement: !!inc.is_reimbursement
                });
            }
        });
        (currentBackupData.expenses || []).forEach(exp => {
